...
- Queue song titles overflow: in `frontend/src/components/QueuePanel.jsx`, keep `min-w-0` on the text container and `truncate` on the title line to avoid horizontal scrolling.
- Backend service `EADDRINUSE`: `kjdj-backend.service` must invoke `backend/scripts/start-backend.sh`, which kills stale `node src/server.js` listeners on the configured port before running `npm start`. After updating the unit, run `sudo systemctl daemon-reload` and restart the service.
- Volume slider no-op: the fix in `frontend/src/components/KaraokePlayer.jsx` applies the slider value to both audio and video tags on every change. If the issue reappears, ensure the `useEffect` that sets `el.volume = volume` is intact.
